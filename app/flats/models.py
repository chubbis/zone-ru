...
итого: {round(total, 2)}
    ...
    def get_hot_water(hot_water, last_month_hot_water, flat_number):
        ...
        return [
            f"Горячая вода: {hot_water} - {last_month_hot_water}. {hot_water_diff} x {price} = {hot_water_price}",
            hot_water_price,
            hot_water_diff
        ]
    ...
    def get_cold_water(cold_water, last_month_cold_water, flat_number):
        ...
        return [
            f"Холодная вода: {cold_water} - {last_month_cold_water}. {cold_water_diff} x {price} = {cold_water_price}",
            cold_water_price,
            cold_water_diff
        ]
    ...
    def get_day_el(day_el, last_month_day_el, flat_number):
        ...
        return (
            f"День: {day_el} - {last_month_day_el}. {day_el_diff} x {price} = {day_el_price}",
            day_el_price
        )
    ...
    def get_night_el(night_el, last_month_night_el, flat_number):
        ...
        return f"Ночь: {night_el} - {last_month_night_el}. {night_el_diff} x {price} = {night_el_price}", night_el_price
    ...
    def get_water_drain(water_sum, flat_number):
        ...
        return f"Водоотведение: {water_sum} * {price} = {water_drain_price}", water_drain_price
    ...
    def get_water_heating(hot_water, flat_number):
        ...
        return f"Водонагрев: {price} * 0,051 * {hot_water} = {water_heating_price}", water_heating_price
